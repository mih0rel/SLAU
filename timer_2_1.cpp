...
template<typename T>
std::vector<double> chebyshev(CSR<T>& matrix, const std::vector<T>& b, const std::vector<T>& x_s, int r, double lambda_min, double lambda_max, long double break_condition, int max_iter){
    std::ofstream file;
    file.open("Chebyshev.csv");
    file << "iter,residual,time_us" << std::endl;
    ...
    file.close();
    std::cout << "Chebyshev iterations = " << k << ", residual = " << r_cur << std::endl;
    return x_0;
}
...
int main(){
    ...
    std::vector<double> x_3 = chebyshev(mat, b, x, 5, 1, lambda_max, break_condition, max_iter);
    std::vector<double> x_4 = jacobi(mat, b, x, break_condition, max_iter);
