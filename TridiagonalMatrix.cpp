...
void TridiagonalMatrix::check_size_consistency_() const {
    if (a_.size() != b_.size() || c_.size() != b_.size()) {
        throw std::invalid_argument("TridiagonalMatrix: a,b,c must have the same size.");
    }
}
...
std::vector<double> TridiagonalMatrix::mul(const std::vector<double>& x) const {
    ...
    for (std::size_t i = 1; i + 1 < n; ++i) {
        y[i] = a_[i] * x[i - 1] + b_[i] * x[i] + c_[i] * x[i + 1];
    }

    // i=n-1
    y[n - 1] = a_[n - 1] * x[n - 2] + b_[n - 1] * x[n - 1];
